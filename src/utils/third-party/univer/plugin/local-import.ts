...
export class LocalImportButtonPlugin extends Plugin {
  ...
  override onStarting(): void {
    ...
    const menuItemFactory = (accessor: IAccessor) => {
      const univerAPI = FUniver.newAPI(accessor.get(Injector));
      const isDoc = !!univerAPI.getActiveDocument?.();
      const isSheet = !!univerAPI.getActiveWorkbook?.();

      let tooltipKey = 'parker-vue-lab-plugins.local-import.tooltip';
      if (isSheet) {
        tooltipKey = 'parker-vue-lab-plugins.local-import.tooltipSheet';
      } else if (isDoc) {
        tooltipKey = 'parker-vue-lab-plugins.local-import.tooltipDoc';
      }
